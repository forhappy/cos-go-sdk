...
- COS Go SDK 提供了三套接口，共 39 个接口，所有接口又细分为: 1. 普通接口，2. 异步接口，所谓异步接口，即调用此类接口时调用方会立即返回一管道，此后调用方可以读取改管道获取返回接口，3. 基于回调函数的接口，即调用此类接口时调用方需提供一个回调函数，调用时此类接口也会立即返回，当任务完成后会自动调用回调函数。
